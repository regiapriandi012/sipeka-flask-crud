...
@app.route("/admin/validasi/ktp/<nik>/<nama>/divalidasi/update", methods=['GET', 'POST'])
@login_required
def ktp_divalidasi_data_update(nik, nama):
    data_ktp = DataKtp.query.get(nik)
    ktp_form = InputKtpUpdate(csrf_enabled=False)
    if ktp_form.validate_on_submit():
        ttl = ktp_form.ttl.data
        ...
        try:
            db.session.commit()
            return redirect(url_for("update_data_ktp_divalidasi_success", nik=nik, nama=nama, _external=True))
        except:
            db.session.rollback()
    return render_template("dataKTP_divalidasi_update.html", data_ktp=data_ktp, ktp_form=ktp_form)

@app.route("/admin/validasi/laporan/<nik>/<nama>/divalidasi/update", methods=['GET', 'POST'])
@login_required
def laporan_divalidasi_data_update(nik, nama):
    data_laporan = DataLaporan.query.get(nik)
    laporan_form = InputLaporanUpdate(csrf_enabled=False)
    if laporan_form.validate_on_submit():
        rt = laporan_form.rt.data
        ...
        try:
            db.session.commit()
            return redirect(url_for("update_data_laporan_divalidasi_success", nik=nik, nama=nama, _external=True))
        except:
            db.session.rollback()
    return render_template("dataLaporan_divalidasi_update.html", data_laporan=data_laporan, laporan_form=laporan_form)
...
def update_data_laporan_divalidasi_delete(nik, nama):
    data_laporan = DataLaporan.query.get(nik)
    try:
        db.session.delete(data_laporan)
        db.session.commit()
    except:
        db.session.rollback()
    return render_template("dataLaporan_divalidasi_delete.html", nama_form=nama, nik_form=nik)
